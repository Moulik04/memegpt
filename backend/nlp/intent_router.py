...
USE_WHEN: dict[str, str] = {
    ...
    "always_has_been":         "IRONIC 'IT ALWAYS WAS': Revealing that a surprising or dark truth was secretly true the entire time, recontextualizing everything that came before. E.g. realizing the 'random' outage was actually planned maintenance nobody announced. A single flat reveal, not a gradual escalation; NOT unmasking a specific hidden culprit (scooby_doo_mask_reveal) and NOT confronting an uncomfortable truth about yourself (the_scroll_of_truth).",
    ...
    "one_does_not_simply":     "SECRETLY HARD TASK: Declaring that something everyone assumes is easy is actually very difficult — a solemn, mock-epic warning. E.g. 'one does not simply merge without a code review,' or 'one does not simply skip leg day.' A single declarative statement, not a comparison or dialogue; NOT a debatable opinion stated as fact (change_my_mind).",
    ...
    "monkey_puppet":           "AWKWARD SIDE-EYE: Silently glancing away, unsure how to react to something uncomfortable just said or done — the awkwardness IS the joke, no words needed. E.g. the room going quiet after an inappropriate joke. NOT wordless profound disappointment (disappointed_black_guy) and NOT being caught together in public (kiss_cam_caught) — this is quiet discomfort, not a verdict or an exposure.",
    "oprah":                   "EVERYBODY GETS ONE: 'You get an X! You get an X!' — distributing the same thing to literally everyone in the room, generous or absurdly indiscriminate. E.g. every team getting blamed equally for one person's mistake. NOT unanimous collective rejection of one thing (all_my_homies_hate) — this is everyone RECEIVING something, not agreeing against it.",
    ...
    "trade_offer":             "LOPSIDED TRADE PROPOSAL: 'I receive X. You receive Y.' — a flatly stated, wildly unbalanced trade, presented as if it's a fair deal. E.g. 'I receive credit for the project. You receive the 2am debugging session.' Ironic or one-sided exchanges. NOT a disappointing counter in an actual negotiation (pawn_stars_best_i_can_do) — this is a flat statement, not a response to an ask.",
    ...
    "drunk_friend_caught":     "Someone caught on camera with a dazed, confused, blackout-adjacent stare — 'wait what's happening'; perfect for drunk friend moments, being caught off guard, pretending to be sober, POV phone-in-face surprise, or general dissociation at a social event. NOT a knowing side-eye reaction to something someone else said (monkey_puppet) and NOT being caught together with someone else (kiss_cam_caught) — this is solo disorientation.",

    # --- Full catalog — file-stem-matched keys so ChromaDB lookups resolve correctly ---
    "epic_handshake":          "UNLIKELY COMMON GROUND: Two rivals, enemies, or opposites shaking hands because they discover they agree on exactly one specific thing, despite disliking everything else about each other. E.g. two feuding coworkers bonding over hating the same broken tool. NOT continued disagreement (american_chopper_argument, woman_yelling_at_cat) — this requires an actual moment of agreement.",
    ...
    "uno_draw_25_cards":       "AVOIDANCE AT ABSURD COST: Offered one small reasonable action ('just do/say/admit X') and choosing a massive self-inflicted penalty instead. Pure refusal — there is NO tempting alternative being chased. NOT for a tempting distraction pulling you away (distracted_boyfriend) or a last-second swerve toward something else (left_exit_12) — both of those chase something; this chooses nothing.",
    ...
    "all_my_homies_hate":      "UNANIMOUS GROUP DISDAIN: 'All my homies hate X' — a flat rallying cry declaring collective, unanimous rejection of one specific thing. E.g. 'all my homies hate daylight saving time,' or 'all my homies hate surprise meetings.' NOT everyone receiving something (oprah) — this is collective rejection, not distribution.",
    ...
    "flex_tape":               "OVERKILL DUCT-TAPE FIX: A wildly overconfident, disproportionate fix applied to a problem — maximum bravado, minimum actual engineering. E.g. duct-taping over a production bug instead of fixing the root cause. NOT a sincerely-delivered fake 'genius' solution (baburao) — this is knowingly slapping tape on it, not mistaking the tape for a real fix.",
    "marked_safe_from":        "TREATING A MINOR THING LIKE A DISASTER: 'Marked safe from X' — using disaster-level language for a trivial inconvenience, played completely straight. E.g. 'marked safe from Monday's stand-up meeting,' or 'marked safe from the office coffee machine breaking again.' NOT an actual visible disaster (a_train_hitting_a_school_bus) — the whole joke is that nothing serious happened.",
    ...
    "running_away_balloon":    "CHASING THE THING THAT KEEPS ESCAPING: Desperately chasing after something that keeps floating just out of reach — label the balloon with what you're chasing. E.g. chasing 'work-life balance,' or chasing a promotion that keeps getting pushed back. NOT desperate craving for more of something you already have (y_all_got_any_more_of_that) — this is chasing something you don't have and can't quite catch.",
    "absolute_cinema":         "PEAK [THING], SERIOUSLY OR IRONICALLY: Declaring something 'this is peak cinema' — either genuinely impressive or ironically overhyped for something mundane. E.g. calling a beautifully organized spreadsheet 'peak cinema,' or genuinely praising a great piece of work this way. NOT a personal toast to one specific moment you spotted (leonardo_dicaprio_cheers) — this is a broader declaration about overall quality, not a satisfied reaction.",
    ...
    "star_wars_yoda":          "INVERTED MOCK-WISDOM: Yoda's backwards sentence structure used for mock-profound, exaggeratedly wise-sounding advice. E.g. 'Deploy on Friday, you must not,' or 'Debug it myself, I will.' NOT clever-sounding but flawed reasoning (roll_safe_think_about_it) — this is just backwards phrasing, the logic itself is fine.",
    "the_rock_driving":        "SUDDEN REAR-VIEW DOUBLE-TAKE: Casually driving along, then suddenly noticing something unexpected behind you and doing a surprised double-take. E.g. suddenly realizing a forgotten task is due today. NOT skeptical doubt about a claim (third_world_skeptical_kid) — this is genuine startled noticing, not suspicion.",
    ...
    "but_that_s_none_of_my_business": "PASSIVE-AGGRESSIVE TEA-SIPPING OBSERVATION: Kermit sipping tea while pointedly observing something about someone else, then disclaiming involvement — 'but that's none of my business.' E.g. quietly noting a coworker takes credit for others' work, then pointedly saying nothing further. NOT envious or resentful watching (squidward_window) — this is smug judgment with a disclaimer, not envy.",
    "finding_neverland":       "COLLECTIVE TEARFUL DEVASTATION: An entire audience or group reduced to tears together — something hit so hard everyone's crying, sincerely or dramatically overplayed. E.g. the whole team getting emotional at a heartfelt goodbye message. NOT a group's shared anger or rejection (all_my_homies_hate) — this is shared sadness, not disdain.",
    ...
    "three_headed_dragon":     "THREE UNEXPECTED ALLIES: A three-headed dragon where all three heads unexpectedly agree — for labeling three separate groups who unexpectedly share the exact same belief. E.g. three completely different departments all secretly agreeing the new policy is bad. NOT two extremes agreeing against a middle position (midwit_bell_curve) — this is three equal parties, not a horseshoe.",
    "they_don_t_know":         "SECRETLY DIFFERENT AT THE PARTY: Standing in a crowd while internally noting 'they don't know I [secret]' — feeling quietly superior, burdened, or different while blending in. E.g. attending a meeting while secretly already knowing the outcome. NOT a visual thought-bubble exposing someone ELSE's contradicting thoughts (i_bet_he_s_thinking_about_other_women) — this is your own unspoken internal narration.",
    "y_all_got_any_more_of_that": "DESPERATE FOR MORE: Wide-eyed, can't-get-enough desperation for more of something — needy, hooked, please-give-me-more energy. E.g. desperately wanting more of a great song on repeat. NOT chasing something you don't have yet and can't catch (running_away_balloon) — this is craving more of something you already have.",
    "soldier_protecting_sleeping_child": "FIERCELY GUARDING SOMETHING VULNERABLE: A soldier shielding a sleeping child from an approaching threat — protecting something innocent or vulnerable from encroaching danger. E.g. protecting a junior teammate from unfair blame. NOT hiding from a threat yourself (anime_girl_hiding_from_terminator) — this is shielding someone else, not self-preservation.",
    "pawn_stars_best_i_can_do": "DISAPPOINTING LOWBALL COUNTER-OFFER: 'Best I can do is X' — a flat, unbothered counter that's way less than what was hoped for or asked. Requires an actual back-and-forth negotiation or ask; NOT shock at an unexpected consequence (surprised_pikachu) — this is a deliberate, calm lowball, not a surprised reaction. NOT a flatly-stated one-sided trade with no negotiation (trade_offer).",
    ...
    "disappointed_black_guy":  "WORDLESS PROFOUND DISAPPOINTMENT: A flat, silent, deeply knowing disappointment — no explanation needed, you saw this coming and it still hurts. E.g. the look after a promised feature ships broken again. NOT awkward unsure side-eye (monkey_puppet) — this is a stoic, knowing verdict, not confusion.",
    "inhaling_seagull":        "WINDING UP TO SAY SOMETHING UNHINGED: Taking a huge, dramatic breath before unleashing something chaotic, too honest, or completely unfiltered. E.g. the deep breath before finally telling a coworker what you really think. NOT an ongoing back-and-forth argument (american_chopper_argument) — this is the single wind-up moment before anything is said.",
    ...
    "no_yes":                  "CLEAN BINARY SPLIT: A simple No/Yes contrast — one thing firmly rejected, another eagerly accepted, side by side. E.g. 'No to Monday meetings. Yes to Friday deploys.' NOT the Drake reject/approve image format (drake) — this is a plain word-for-word label, no character imagery involved.",
    ...
    "whe_i_m_in_a_competition_and_my_opponent_is": "EFFORT MISMATCH VS COMPETITION: A side-by-side showing a huge gap in effort, skill, or preparation between you and whoever you're up against. E.g. showing up with a rough draft against a competitor's polished pitch. NOT a past-vs-present strength comparison (buff_doge_vs_cheems) — this is a same-moment gap against a rival, not decay over time.",
    "where_monkey":            "'WHERE MONKEY?': Confused, urgent searching for something that was expected to be there and is suddenly, bewilderingly missing. E.g. searching for a file that was definitely saved. NOT suddenly seeing something everywhere (x_x_everywhere) — this is the opposite: something that should be there is nowhere to be found.",
    ...
    "two_guys_on_a_bus":       "QUIET CONSPIRATORIAL PLANNING: Two people leaning in together, whispering — secret scheming or quiet mutual agreement away from everyone else. E.g. two coworkers quietly agreeing to push back on a bad decision together. NOT one person's solo scheming (circuit_plan) — this specifically requires two people whispering together.",
    "x_x_everywhere":          "ONCE YOU SEE IT, YOU CAN'T UNSEE IT: Buzz Lightyear — 'X, X everywhere' — noticing a pattern that then seems to appear inescapably everywhere. E.g. noticing one typo and then seeing typos everywhere. NOT urgently searching for something missing (where_monkey) — this is the opposite: something unwanted keeps appearing.",
    "you_guys_are_getting_paid": "DISCOVERING EVERYONE ELSE GOT SOMETHING: Tobey Maguire's surprised face — 'you guys are getting paid?' — realizing everyone else had access to a benefit, bonus, or perk you didn't know about. E.g. finding out the whole team got extra time off you weren't told about. NOT shock that someone else is MISSING something common (megamind_no_bitches) — this is shock that everyone else secretly HAD something you didn't.",
    "a_train_hitting_a_school_bus": "UNSTOPPABLE VISIBLE DISASTER: A train about to hit a bus stuck on the tracks — a disaster you can clearly see coming and are powerless to stop. E.g. watching a project deadline collide with an obviously insufficient timeline, brace-for-impact energy. NOT overstated language for something actually trivial (marked_safe_from) — this is a real, serious disaster.",
    "i_m_the_captain_now":     "HOSTILE TAKEOVER: 'Look at me. I am the captain now' — a dead-calm, unsettling declaration of a sudden change in who's in control, seizing ownership of a role someone else was running. E.g. a new hire calmly announcing they're the one reviewing everyone's PRs now. NOT a goofy self-congratulatory win (floss_dance) or pure unbridled hype (party_parrot).",
    "i_bet_he_s_thinking_about_other_women": "REVEALING THE REAL THOUGHT BUBBLE: A person smiling with a visible thought bubble exposing what they're ACTUALLY thinking about, contradicting what they claim. E.g. someone saying they're 'fully focused' while visibly thinking about lunch. NOT your own unspoken internal narration (they_don_t_know) — this is a visual reveal of someone ELSE's contradicting thoughts.",
    ...
    "george_bush_9_11":        "THE MOMENT EVERYTHING CHANGED: 'And then the towers fell' — a specific, date-stamped moment after which everything was different, before/after framing. E.g. 'and then the outage happened' as the dividing line for a whole quarter's plans. NOT revealing something was secretly true all along (always_has_been) — this is a real dividing event, not a hidden-truth reveal.",
    "0_days_without_lenny_simpsons": "SAFETY COUNTER RESET AGAIN: A safety counter reset back to 0 — something went wrong again, and the streak of avoiding it just ended. E.g. '0 days without a merge conflict,' resetting yet again. NOT weary resignation at a recurring bad situation (ah_shit_here_we_go_again) — this is specifically a numeric streak resetting to zero.",
    ...
    "anime_girl_hiding_from_terminator": "HIDING FROM SOMETHING ACTIVELY HUNTING YOU: Hiding behind a pole or object from an approaching threat — genuine dread of something dangerous closing in. E.g. hiding from a manager who's looking for whoever broke the build. NOT shielding someone ELSE from danger (soldier_protecting_sleeping_child) — this is your own self-preservation.",
    "bugs_bunny_communist":    "IRONIC CHAOTIC SOLUTION: Bugs Bunny in a Soviet hat — 'allow me to introduce some communism' — proposing an ironic, chaotic-neutral, collectivist fix to a problem. E.g. jokingly proposing everyone just shares one login instead of fixing permissions properly. NOT sincerely proposing a bad idea as if it's genius (baburao) — this is a knowingly ironic joke, not overconfident wrongness.",
    "say_the_line_bart_simpsons": "CROWD DEMANDS THE CATCHPHRASE: 'Say the line, Bart!' — a crowd or group demanding someone perform their predictable, expected line or move on cue. E.g. the team demanding the same joke or comment someone always makes in stand-up. NOT general craving for more of something (y_all_got_any_more_of_that) — this demands one specific, predictable line.",
    ...
    "theyre_the_same_picture": "FALSELY CLAIMED IDENTICAL: Holding up two photos that are actually the same, insisting anyone who distinguishes them is wrong. E.g. two job postings with wildly different titles but identical responsibilities. NOT confidently mislabeling one single thing (is_this_a_pigeon) — this requires two things being falsely claimed identical.",
    "third_world_skeptical_kid": "SUSPICIOUS OF SOMETHING TOO CONVENIENT: A skeptical raised eyebrow at a claim that sounds implausibly convenient or too good to be true. E.g. side-eyeing a 'quick 5-minute fix' estimate. NOT genuine startled noticing (the_rock_driving) — this is doubt, not surprise.",
    "this_is_where_i_d_put_my_trophy_if_i_had_one": "EMPTY SPACE FOR SOMETHING NEVER EARNED: An empty shelf or display case — 'this is where I'd put X, if I had one' — wanting something you've never managed to get. E.g. an empty spot on the resume for an award you never won. NOT actively chasing something still attainable (running_away_balloon) — this is resigned acceptance of never having earned it.",
    "aj_styles_undertaker":    "LEGENDARY UNEXPECTED CONFRONTATION: Two legendary rivals meeting unexpectedly — a confrontation between two giants from completely different worlds. E.g. two completely unrelated problems colliding at the worst possible time. NOT rivals finding unexpected agreement (epic_handshake) — this is an unexpected clash, not common ground.",
    "grant_gustin_over_grave": "UNBOTHERED ABOUT IMPENDING DOOM: Cheerfully posing over your own grave — surprisingly at peace with a terrible, doom-laden outcome. E.g. calmly accepting a project is doomed while still smiling for the retro slide. NOT active calm composure while still handling an ongoing crisis (dhoni_calm) — this is cheerful acceptance of an already-lost cause.",
    ...
    "mother_ignoring_kid_drowning_in_a_pool": "DANGEROUSLY DISTRACTED BY SOMETHING ELSE: Completely absorbed in one thing (a phone call) while something urgent unfolds right next to you, unnoticed. E.g. scrolling social media while a production alert quietly piles up, oblivious to the urgency. NOT a casual conversation happening during the crisis (empire_state_climbers) — this is silent, oblivious distraction, not chatting through it.",
    ...
    "empire_state_climbers":   "CASUAL CHAT MID-CRISIS: Two people having a totally unrelated, relaxed side conversation while climbing toward a high-stakes crisis. E.g. calmly discussing lunch plans in the middle of an active production incident. NOT silent, oblivious distraction by something unrelated (mother_ignoring_kid_drowning_in_a_pool) — this is an active conversation, not silence.",
    "shut_up_and_take_my_money": "INSTANT MUST-HAVE ENERGY: Fry from Futurama throwing money at the screen — wanting something so badly you'd pay anything for it immediately. E.g. seeing a tool that finally fixes your exact workflow problem and wanting it instantly. NOT craving more of something already loved (y_all_got_any_more_of_that) — this is instant desire for something brand new.",
    ...
    "math_lady":                "FRANTIC MENTAL MATH: Actively trying to calculate, decode, or figure something out in real time while visibly lost — floating equations energy. For effortful live confusion; NOT skeptical squinting at a claim (futurama_fry).",
    ...
    "turkish_shooter":          "EFFORTLESS PRO: Casually achieving with zero equipment, prep, or visible effort what everyone else needs full gear and sweat for — hand-in-pocket mastery. For minimal-effort excellence and 'built different' calm. NOT just a nonchalant attitude with no real achievement (chill_guy) — this requires actually accomplishing something impressive.",
    "well_yes_but_actually_no": "TECHNICALLY-TRUE REVERSAL: Something that is technically correct yet completely wrong in practice or spirit — the baked-in 'Well yes, but actually no' is the answer. Only fill the claim being answered; NOT for corrections mid-sentence (batman_slapping_robin).",
    ...
    "sad_hamster":              "PATHETIC PLEADING EYES: Feeling small, rejected, or pitiful and milking it for sympathy — theatrical puppy-dog-eyes self-pity over something minor. For cute performative sadness; NOT stoic silent disappointment (disappointed_black_guy) — this is theatrical and attention-seeking, not quietly knowing.",
    ...
    "raju_hera_pheri":  "DESI SWAGGER, UNBOTHERED CONFIDENCE: Raju (Akshay Kumar) in his colourful shirt — cool, stylish, unbothered energy walking into any situation. E.g. strolling into a chaotic meeting completely unfazed, 'main hoon na' confidence. NOT calm composure specifically under active pressure (dhoni_calm) and NOT the generic Western nonchalance of not caring (chill_guy) — this is stylish swagger, not stoicism.",
    ...
    "dhoni_calm":       "ICE-COLD UNDER PRESSURE: MS Dhoni's legendary calm — completely unbothered composure while everyone else panics under extreme pressure. E.g. calmly debugging a live outage while the whole team is panicking around you. NOT cheerful acceptance of an already-lost cause (grant_gustin_over_grave) — this is active composure while still in the fight.",
    ...
    "jethalal_panic":   "ESCALATING 4-PANEL DREAD: Jethalal — mounting panic across four panels as the consequences of getting caught sink in. E.g. slowly realizing your mistake is about to be discovered by your manager, panel by panel. NOT a calm-then-worse whiplash (panik_kalm_panik) — this is pure escalating dread with no calming middle beat, and NOT slowly doing the dumb thing yourself (clown_applying_makeup) — this is fear of being caught, not self-sabotage.",
    "alliswel":         "FORCED POSITIVITY AMID COLLAPSE: 'All is well' — chanting fake reassurance to yourself and others as things visibly fall apart. E.g. insisting the project is 'totally on track' while the deadline quietly implodes. NOT genuine cheerful acceptance of doom (grant_gustin_over_grave) — this is anxious denial, not peace.",
    "mogambo_khush":    "VILLAIN SATISFACTION WHEN THE PLAN WORKS: Mogambo's sinister grin — evil satisfaction when your plan succeeds, when karma hits, or when you were right all along. E.g. smiling when the thing you warned everyone about finally happens. NOT quiet smug satisfaction at someone ELSE's unrelated misfortune (disaster_girl) — this is satisfaction at YOUR OWN plan or prediction succeeding.",
    ...
    "party_parrot":     "PURE HYPE CELEBRATION: A bobbing rainbow parrot losing its mind with excitement — unbridled, uncomplicated hype with zero nuance. E.g. the whole team losing it when the deploy finally goes green. NOT a goofy deliberate victory dance (floss_dance) — this is pure hype, not an ironic performance.",
    ...
    "bernie_i_am_once_again_asking_for_your_support": "RELUCTANT RE-ENGAGEMENT: A weary, resigned admission that you are forced to repeat a request, question, or task that has already been ignored or failed previously; conveys a sense of 'here we go again' exhaustion. E.g. once again asking teammates to write tests for their PRs. NOT for a sudden realization of a mistake (surprised_pikachu), a plan backfiring (grus_plan), or a simple preference (drake).",
    ...
    "c_mon_do_something": "SILENT ACCUSATORY POINTING: A crude, masked, deadpan figure stands off to the side pointing wordlessly at a specific action or event just offscreen — no argument, no dialogue, just damning silence. E.g. calling out a coworker who reopened the same bug three sprints in a row. NOT for two people arguing (woman_yelling_at_cat) and NOT for a generic shocked reaction (surprised_pikachu).",
...
{few_shot_block}{avoid_block}{humor_block}{lexicon_block}Templates:
{template_catalog}
...
async def parse_intent(
    ...
    lexicon: Growth Phase C opt-in Lore lexicon — short extracted phrases
    (names/nicknames/running jokes) for this anon user, used for callback
    humor only if genuinely relevant. Only ever reaches the prompt through
    this instruction channel, never through user_message/situation text.
...
async def _parse_intent_inner(
    ...
    if lexicon:
        lexicon_block = (
            f"This group's recurring names/running jokes — use for a callback only "
            f"if genuinely relevant, never force it: {', '.join(lexicon)}.\n\n"
        )
